...
## [ERR-20260714-007] animation-source-assertion

**Logged**: 2026-07-14T17:45:00+08:00
**Priority**: low
**Status**: resolved
**Area**: tests

### Summary
A source-level animation assertion failed because its regular expression omitted the quoted `.nav` selector syntax.

### Error
```
Error: missing all five navigation reveals
```

### Context
- The `revealMap` already contained `.nav` for all five screens.
- The test pattern did not correctly match the JavaScript array strings.

### Suggested Fix
Assert each screen mapping independently with an expression that includes the quoted selector.
...
### Resolution
- **Resolved**: 2026-07-14T17:46:00+08:00
- **Notes**: Replaced the aggregate expression with five explicit mapping assertions.

---

## [ERR-20260714-006] browser-runtime-bootstrap

**Logged**: 2026-07-14T17:40:00+08:00
**Priority**: low
**Status**: pending
**Area**: tests

### Summary
The in-app browser runtime failed to initialize while preparing local mobile animation QA.
...
### Context
- Operation: initialize the bundled browser client and select the local HTTP mobile page.
- The local page server started normally and source-level HTML/JavaScript validation remained available.

### Suggested Fix
Reset the persistent JavaScript browser session and retry initialization; if the runtime conflict persists, use source-level transition assertions and static DOM checks.
